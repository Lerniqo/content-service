...
export class Neo4jService {
    ...
    async onModuleDestroy() {
        try {
            ...
            LoggerUtil.logInfo(this.logger, 'Neo4jService', 'Neo4j driver closed successfully');
        } catch (error) {
            LoggerUtil.logError(this.logger, 'Neo4jService', 'Error closing Neo4j driver', error);
        }
    }

    getSession(): Session {
        const session = this.driver.session();
        LoggerUtil.logDebug(this.logger, 'Neo4jService', 'Created new database session');
        return session;
    }
    ...
    async read(cypher: string, params?: Record<string, any>): Promise<any> {
        ...
        try {
            LoggerUtil.logDebug(
                ...
                { query: cypher.substring(0, 100) + (cypher.length > 100 ? '...' : ''), params }
            );
